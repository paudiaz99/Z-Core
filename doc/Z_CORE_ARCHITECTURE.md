...
- **Data Cache**: Z-Core includes a parameterizable 2-way set-associative data cache (`rtl/z_core_data_cache.v`) with a write-back, write-allocate policy and single cycle throughput. Deploys LRU replacement policy for avoidance of potential conflicts and higher performance.
